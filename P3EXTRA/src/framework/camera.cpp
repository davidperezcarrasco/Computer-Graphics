...
void Camera::updateViewMatrix()
{
	...
	front.normalize(); // Normalizamos el vector front

	Vector3 side = front.cross(up); // Calculamos el vector side = front x up y lo normalizamos
	side.normalize();
	...
	// Construimos la matriz de vista
	view_matrix.setIdentity();
	...
	//view_matrix.setTranslation(-eye.x, -eye.y, -eye.z);
...
void Camera::updateProjectionMatrix()
{
	...
	float f = (1 / tanf((fov / 2) * DEG2RAD)); // Calcular f
	...
	projection_matrix.M[2][0] = 0.0;  projection_matrix.M[2][1] = 0.0; projection_matrix.M[2][2] = (near_plane + far_plane) / (near_plane - far_plane);      projection_matrix.M[2][3] = -1;
	projection_matrix.M[3][0] = 0.0;  projection_matrix.M[3][1] = 0.0; projection_matrix.M[3][2] = 2 * (near_plane * far_plane) / (near_plane - far_plane);  projection_matrix.M[3][3] = 0.0;
